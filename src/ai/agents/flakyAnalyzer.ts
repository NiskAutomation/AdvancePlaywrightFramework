...
const flakySummaryAgent = createAgent<FlakySummaryInput, { summary: string }>({
    name: 'flaky-summary',
    schema,
    prompt: ({ prevId, currId, flips, total }) => `
...
Write a "summary" saying what these tests have in common, the most likely cause of the
...
export async function analyzeFlaky(
    ...
    const result = await flakySummaryAgent({
        prevId: prev.runId,
        currId: curr.runId,
        flips,
        total: allTests.size,
    ...
    return { counts, flaky, summary: result.status === 'success' ? result.data.summary : undefined };
}
