...
const safeAnalyzeFlaky = (() => {
    try {
        const mod = require('../ai/agents/flakyAnalyzer');
        return typeof mod.analyzeFlaky === 'function'
            ? mod.analyzeFlaky
            : async () => ({
                counts: { flaky: 0, failing: 0, total: 0 },
                flaky: [],
                summary: 'Flaky analysis is unavailable because the optional analyzer module is not installed.',
            } as FlakyResult);
    } catch {
        return async () => ({
            counts: { flaky: 0, failing: 0, total: 0 },
...
const safeHasApiKey = (() => {
    try {
        const mod = require('../ai/config/providers');
        return typeof mod.hasApiKey === 'function' ? mod.hasApiKey : () => false;
    } catch {
        return () => false;
    }
})();

const analyzeFailure = safeAnalyzeFailure;
const analyzeFlaky = safeAnalyzeFlaky;
const hasApiKey = safeHasApiKey;
...
class CustomReporter implements Reporter {
    ...
    private async runFlakyAnalysis(): Promise<void> {
        ...
        const flakyResult = await analyzeFlaky(prev, curr, hasApiKey());
        const resolvedFlakyResult: FlakyResult = flakyResult ?? {
            counts: { flaky: 0, failing: 0, total: 0 },
            flaky: [],
            summary: 'Flaky analysis returned no result.',
        };
        this.flakyResult = resolvedFlakyResult;
        this.prevBuildId = prev.runId;
        this.currBuildId = curr.runId;

        const flakyCounts = resolvedFlakyResult.counts ?? { flaky: 0, failing: 0, total: 0 };
        console.log(
            `🔁 Flaky analyzer: ${flakyCounts.flaky} flaky, ${flakyCounts.failing} failing (vs build ${prev.runId}).`,
        );
    }
...
<body>
    ...
    <footer class="report-footer">
        <p>Built with ❤️ by <a href="https://thetestingacademy.com" target="_blank">Nishikant Pradhan</a> | <a href="https://thetestingacademy.com" target="_blank">The Testing Academy</a></p>
    </footer>
